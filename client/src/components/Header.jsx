...
function Header() {
    ...
    // Define the queries with console logs in the onCompleted and onError callbacks
    const [searchUsers, { data: usersData, loading: loadingUsers, error: userError }] = useLazyQuery(SEARCH_USERS, {
        onCompleted: data => {
    ...
    const [searchSchedules, { data: schedulesData, loading: loadingSchedules, error: scheduleError }] = useLazyQuery(SEARCH_SCHEDULES, {
        onCompleted: data => {
            console.log("Schedules search completed successfully:", data);
            navigate('/search', { state: { users: usersData, schedules: schedulesData } });
        },
        onError: error => {
